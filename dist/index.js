...
server.tool('linklee_create_link', 'Create a short link on Linklee. Returns the short URL and link details.', {
    ...
    const body = { targetUrl: args.targetUrl };
    if (args.key)
        body.key = args.key;
    if (args.utmSource)
        body.utmSource = args.utmSource;
    if (args.utmMedium)
        body.utmMedium = args.utmMedium;
    if (args.utmCampaign)
        body.utmCampaign = args.utmCampaign;
    if (args.comments)
        body.comments = args.comments;
    const result = await apiRequest(`/workspaces/${args.workspaceId}/links`, { method: 'POST', body });
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
server.tool('linklee_list_links', 'List short links in a workspace with optional search and pagination.', {
    workspaceId: z.string().describe('Workspace UUID'),
...
server.tool('linklee_get_analytics', 'Get click analytics: clicks, geography, devices, browsers, referrers.', {
    workspaceId: z.string().describe('Workspace UUID'),
...
server.tool('linklee_list_tags', 'List tags in a workspace.', {
    workspaceId: z.string().describe('Workspace UUID'),
}, async (args) => {
